...
import (
	...
	"golang-api-starter/internal/ws"
...
var custMiddlewares = middleware.NewCustMiddlewares()
...
func (f *Fiber) LoadWebsocket() {
	cfg.WatchConfig()

	f.App.Use("/ws", func(c *fiber.Ctx) error {
	...
	router := f.App.Group("/ws",
		custMiddlewares.Log(),                           // add logging to all routes
		custMiddlewares.CheckJwt(skipJwtCheckRoutes...), // add jwt check to all routes
	)

	ws.ServeCurrLoginUserWs(router)
}
