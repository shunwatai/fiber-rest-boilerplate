...
func (c *Controller) Get(ctx *fiber.Ctx) error {
	logger.Debugf("groupResourceAcl ctrl\n")
...
func (c *Controller) Create(ctx *fiber.Ctx) error {
	logger.Debugf("groupResourceAcl ctrl create\n")
	c.service.ctx = ctx
	groupResourceAcl := &GroupResourceAcl{}
	groupResourceAcls := []*GroupResourceAcl{}
	...
	groupResourceAclErr, parseErr := reqCtx.Payload.ParseJsonToStruct(groupResourceAcl, &groupResourceAcls)
	if parseErr != nil {
		return fctx.JsonResponse(
			fiber.StatusUnprocessableEntity,
	...
	if groupResourceAclErr == nil {
		groupResourceAcls = append(groupResourceAcls, groupResourceAcl)
	}
	// logger.Debugf("groupResourceAclErr: %+v, groupResourceAclsErr: %+v\n", groupResourceAclErr, groupResourceAclsErr)
	// for _, t := range groupResourceAcls {
	// 	logger.Debugf("groupResourceAcls: %+v\n", t)
	// }
	...
	for _, groupResourceAcl := range groupResourceAcls {
		...
		if groupResourceAcl.Id == nil {
			continue
		} else if existing, err := c.service.GetById(map[string]interface{}{
			"id": groupResourceAcl.GetId(),
		}); err == nil && groupResourceAcl.CreatedAt == nil {
			groupResourceAcl.CreatedAt = existing[0].CreatedAt
		}
		// logger.Debugf("groupResourceAcl? %+v\n", groupResourceAcl)
	}

	// return []*GroupResourceAcl{}
	results, httpErr := c.service.Create(groupResourceAcls)
	if httpErr.Err != nil {
		return fctx.JsonResponse(
			httpErr.Code,
	...
	if groupResourceAclErr == nil && len(results) > 0 {
		return fctx.JsonResponse(
			respCode,
...
func (c *Controller) Update(ctx *fiber.Ctx) error {
	logger.Debugf("groupResourceAcl ctrl update\n")
	...
	for _, groupResourceAcl := range groupResourceAcls {
		...
		if groupResourceAcl.Id == nil && groupResourceAcl.MongoId == nil {
			return fctx.JsonResponse(
				respCode,
	...
	results, httpErr := c.service.Update(groupResourceAcls)
	if httpErr.Err != nil {
		return fctx.JsonResponse(
			httpErr.Code,
			map[string]interface{}{"message": httpErr.Err.Error()},
...
func (c *Controller) Delete(ctx *fiber.Ctx) error {
	logger.Debugf("groupResourceAcl ctrl delete\n")
