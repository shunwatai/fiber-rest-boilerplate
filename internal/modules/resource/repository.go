...
func cascadeFields(resources Resources) {
  if len(resources) == 0 {
    return
  }
	// cascade user
...
func (r *Repository) Get(queries map[string]interface{}) ([]*Resource, *helper.Pagination) {
	logger.Debugf("resource repo get")
	...
	queries["columns"] = Resource{}.getTags()
...
func (r *Repository) Create(resources []*Resource) ([]*Resource, error) {
	for _, resource := range resources {
		logger.Debugf("resource repo add: %+v", resource)
	}
	database.SetIgnoredCols("search")
	defer database.SetIgnoredCols()
	rows, err := r.db.Save(Resources(resources))
...
func (r *Repository) Update(resources []*Resource) ([]*Resource, error) {
	logger.Debugf("resource repo update")
	rows, err := r.db.Save(Resources(resources))
...
func (r *Repository) Delete(ids []string) error {
	logger.Debugf("resource repo delete")
	err := r.db.Delete(ids)
	if err != nil {
		return err
	}
