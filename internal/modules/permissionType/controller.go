...
func (c *Controller) Get(ctx *fiber.Ctx) error {
	logger.Debugf("permissionType ctrl\n")
...
func (c *Controller) Create(ctx *fiber.Ctx) error {
	logger.Debugf("permissionType ctrl create\n")
	c.service.ctx = ctx
	...
	permissionTypeErr, parseErr := reqCtx.Payload.ParseJsonToStruct(permissionType, &permissionTypes)
	if parseErr != nil {
		return fctx.JsonResponse(
			fiber.StatusUnprocessableEntity,
	...
	if permissionTypeErr == nil {
		permissionTypes = append(permissionTypes, permissionType)
	}
	// logger.Debugf("permissionTypeErr: %+v, permissionTypesErr: %+v\n", permissionTypeErr, permissionTypesErr)
	// for _, t := range permissionTypes {
	// 	logger.Debugf("permissionTypes: %+v\n", t)
	// }
	...
	for _, permissionType := range permissionTypes {
		...
		if permissionType.Id == nil {
			continue
		} else if existing, err := c.service.GetById(map[string]interface{}{
			"id": permissionType.GetId(),
		}); err == nil && permissionType.CreatedAt == nil {
			permissionType.CreatedAt = existing[0].CreatedAt
		}
		// logger.Debugf("permissionType? %+v\n", permissionType)
	}

	// return []*PermissionType{}
	results, httpErr := c.service.Create(permissionTypes)
	if httpErr.Err != nil {
		return fctx.JsonResponse(
			httpErr.Code,
	...
	if permissionTypeErr == nil && len(results) > 0 {
		return fctx.JsonResponse(
			respCode,
...
func (c *Controller) Update(ctx *fiber.Ctx) error {
	logger.Debugf("permissionType ctrl update\n")
	...
	for _, permissionType := range permissionTypes {
		...
		if permissionType.Id == nil && permissionType.MongoId == nil {
			return fctx.JsonResponse(
				respCode,
	...
	results, httpErr := c.service.Update(permissionTypes)
	if httpErr.Err != nil {
		return fctx.JsonResponse(
			httpErr.Code,
			map[string]interface{}{"message": httpErr.Err.Error()},
...
func (c *Controller) Delete(ctx *fiber.Ctx) error {
	logger.Debugf("permissionType ctrl delete\n")
