...
func (m *Mongodb) Connect() *sqlx.DB {
	fmt.Printf("connecting to Mongodb... \n")
	// fmt.Printf("Table: %+v\n", m.TableName)
	connectionString := fmt.Sprintf("mongodb://%s:%s@%s:%s/%s?sslmode=disable", *m.User, *m.Pass, *m.Host, *m.Port, *m.Database)
...
func (m *Mongodb) Select(queries map[string]interface{}) (*sqlx.Rows, *helper.Pagination) {
...
func (m *Mongodb) Save(records Records) *sqlx.Rows {
